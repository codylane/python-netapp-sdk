...
''' %(time.ctime())
    ...
    print('')
    print('def apicall_to_dict(api_call):')
    print("%s'''" %(ws_indent))
    print('%sTakes the apicall result, and transforms it into a dictionary' %(ws_indent))
    print("%s'''" %(ws_indent))
    print('%sxml = api_call.sprintf()' %(ws_indent))
    print('%sreturn xml_to_dict(xml)' %(ws_indent))
    print('')
